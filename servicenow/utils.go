...
// dynamicColumns:: Returns list coulms for a servicenow object
func dynamicColumns(ctx context.Context, client *servicenow.ServiceNow, servicenowTableName string, p *plugin.Plugin) ([]*plugin.Column, plugin.KeyColumnSlice, map[string]string) {
	// sObjectMeta := client.SObject(servicenowTableName).Describe()
	// if sObjectMeta == nil {
	// 	plugin.Logger(ctx).Error("servicenow.dynamicColumns", fmt.Sprintf("Table %s not present in servicenow", servicenowTableName))
	...
	// if err != nil {
	// 	plugin.Logger(ctx).Error("servicenow.dynamicColumns", "json marshal error %v", err)
	// }
	...
	// if err != nil {
	// 	plugin.Logger(ctx).Error("servicenow.dynamicColumns", "json unmarshal error %v", err)
	// }
	// for _, fields := range servicenowObjectFields {
	// 	if fields["name"] == nil {
	// 		continue
	// 	}
	...
	// return cols, keyColumns, servicenowCols
	return nil, nil, nil
}
